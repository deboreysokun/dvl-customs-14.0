    ...
    def _display_lines_sql_q1(self, partners, date_end, account_type, customer_id=None, agent_staff_id=None):
        ...
        query = """
            SELECT l.id, m.name AS move_id, l.partner_id, l.date, l.name,
                            ...
                            COALESCE(op.name, ot.name) AS shipment_id, 
                            op.operation_type, op.bl_number, op.container_number,
            ...
            LEFT JOIN other_operation ot ON (ot.id = m.other_service_id)
        ...
        query += """ AND (
            ...
            GROUP BY l.id, l.partner_id, m.name, l.date, l.date_maturity, l.name,
                op.name, ot.name, op.operation_type, op.bl_number, op.container_number, p_staff.name, op.commodity,op.inv_pack_number,pod.name, pol.name, pod1.name, fd.name, por.name, op.etd, op.eta, op.etr, incoterm.code, inv_inco.code,
                CASE WHEN l.ref IS NOT NULL
                    THEN l.ref
                    ELSE m.ref
                END,
