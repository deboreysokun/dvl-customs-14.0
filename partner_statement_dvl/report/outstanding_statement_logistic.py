    ...
    def _display_lines_sql_q1(self, partners, date_end, account_type, customer_id=None, agent_staff_id=None):
        ...
        query += """ AND (
            ...
            GROUP BY l.id, l.partner_id, m.name, l.date, l.date_maturity, l.name,
                op.name, op.operation_type, op.bl_number, op.container_number, p_staff.name, op.commodity,op.inv_pack_number,pod.name, pol.name, pod1.name, fd.name, por.name, op.etd, op.eta, op.etr, incoterm.code, inv_inco.code,
                CASE WHEN l.ref IS NOT NULL
                    THEN l.ref
                    ELSE m.ref
                END,
