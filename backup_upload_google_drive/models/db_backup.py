...
class DbBackup(models.Model):
    ...
    def google_drive_upload(self, rec, file_path, bkp_file):
        ...
        for record in conf_ids:
            # GOOGLE DRIVE UPLOAP
            if record.is_upload:
                headers = {"Authorization": "Bearer %s" % (access_token)}
                para = {
                    "name": "%s" % (str(bkp_file)),
                    "parents": ["%s" % (str(record.drive_folder_id))]
                }
                files = {
                ...
                if r.status_code == 200:
                    email_to = ""
                    for rec in record.gdrive_email_notif_ids.mapped('login'):
                        email_to += rec + ','
            ...
            if record.drive_autoremove:
                headers = {'Content-type': 'application/json', 'Accept': 'text/plain'}
                params = {
                    'access_token': access_token,
                    'q': "mimeType='application/%s'" % (record.backup_type),
                ...
                for item in content['files']:
                    ...
                    if delta.days >= record.drive_to_remove:
                        params = {
                            'access_token': access_token
                        }
